...
public class TilingPrefetcher {
	 ...
	 */
	public static Set<Long> fetchTiles(final AffineTransform3D sourceToScreen, final int[] cellDimensions,
			final long[] dimensions, final Dimensions screenInterval, final Interpolation interpolation,
			final RandomAccess<?> cellsRandomAccess) {
		new TilingPrefetcher().fetch();
	}

	private TilingPrefetcher()
	{}
	 ...
	 *            access to the source cells
	 */
	private void fetch(final AffineTransform3D sourceToScreen, final int[] cellDimensions, final long[] dimensions,
			final Dimensions screenInterval, final Interpolation interpolation,
			final RandomAccess<?> gridAccess) {
		...
		final double[] bbMin = new double[] { Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY,
				Double.POSITIVE_INFINITY };
		final double[] bbMax = new double[] { Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY,
		...
		for (gridAccess.setPosition(minCell[2],
				2); gridAccess.getIntPosition(2) <= maxCell[2]; gridAccess.fwd(2)) {
			pSource.move(cellDimensions[2], 2);
			pSource.setPosition((minCell[1] - 1) * cellDimensions[1], 1);
			for (gridAccess.setPosition(minCell[1],
					1); gridAccess.getIntPosition(1) <= maxCell[1]; gridAccess.fwd(1)) {
				...
				for (gridAccess.setPosition(nStart,
						0); gridAccess.getIntPosition(0) <= nStop; gridAccess.fwd(0)) {
					sourceToScreen.apply(pSource, pScreen);
					final double x = pScreen.getDoublePosition(0);
					final double y = pScreen.getDoublePosition(1);
					if ((x + offsetPos[0] >= 0) && (x + offsetNeg[0] < w) && (y + offsetPos[1] >= 0)
							&& (y + offsetNeg[1] < h)) {
						// TODO..
						gridAccess.get();
					}
	 ...
	 */
	private void checkProtoCell(final int[] cellDims, final AffineTransform3D sourceToScreen,
